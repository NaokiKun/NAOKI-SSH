	...
	if command -v $_pkg_ &>/dev/null; then
		if [[ $($_pkg_ --version 2>&1) =~ [0-9]+\.[0-9]+ ]]; then
			if ! awk 'BEGIN { exit ARGV[1] < ARGV[2] }' $BASH_REMATCH $_ver_; then
				printf "%s: error: requires package '%s %s' or newer. / အမှား: အထက်ပါ package သည် ပိုမိုလက်ခံနိုင်သော ဗားရှင်း တစ်ခု လိုအပ်ပါသည်。\n" ${_SHELLBOT_[name]} $_pkg_ $_ver_ 1>&2
				exit 1
			fi
		else
			printf "%s: error: could not obtain version for '%s'. / အမှား: အဆိုပါ package အတွက် ဗားရှင်း ရယူ၍ မရပါ။\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
			exit 1
		fi
	else
...
readonly _ERR_PARAM_REQUIRED_='required option: check that the required parameter(s) or argument(s) are present. / လိုအပ်သော ရွေးချယ်စရာ: လိုအပ်သော parameter(များ) သို့ argument(များ) ရှိ/မရှိ စစ်ဆေးပါ။'
readonly _ERR_TOKEN_UNAUTHORIZED_='unauthorized: check that you have permissions to use the token. / မခွင့်ပြုထားပါ: token ကို အသုံးပြုခွင့် ရှိ/မရှိ စစ်ဆေးပါ။'
readonly _ERR_TOKEN_INVALID_='token inválido: verique o número do token e tente novamente.'
readonly _ERR_BOT_ALREADY_INIT_='action not allowed: the bot has already been initialized. / လုပ်ဆောင်ချက် မခွင့်ပြုပါ: bot ကို ယခုထိ ပြင်ဆင်ပြီး စတင်ထားပါသည်။'
readonly _ERR_FILE_NOT_FOUND_='access failure: could not read the file. / ဝင်ရောက်ရန် မအောင်မြင်ပါ: ဖိုင်ကို ဖတ်၍ မရပါ။'
readonly _ERR_DIR_WRITE_DENIED_='permission denied: cannot write to the directory. / ခွင့်မပြုပါ: directory ထဲသို့ ရေး၍ မရပါ။'
readonly _ERR_DIR_NOT_FOUND_='could not access: directory not found. / ဝင်ရောက်၍ မရပါ: directory မတွေ့ပါ။'
readonly _ERR_FILE_INVALID_ID_='invalid id: file not found. / မမှန်ကန်သော id: ဖိုင် မတွေ့ပါ။'
readonly _ERR_UNKNOWN_='unknown error: an unexpected failure occurred. Report the issue to the developer. / မသိသော အမှား: မမျှော်လင့်ထားသော ပြဿနာ တက်ပွားခဲ့သည်။ ပြဿနာကို developer ထံ သတင်းပို့ပါ။'
readonly _ERR_SERVICE_NOT_ROOT_='acesso negado: requer privilégios de root.'
readonly _ERR_SERVICE_EXISTS_='error creating service: the service name already exists. / service ဖန်တီးရာတွင် အမှား: service အမည် ယခင်တွင် ရှိပြီးသား ဖြစ်ပါသည်。'
...
readonly _ERR_SERVICE_USER_NOT_FOUND_='user not found: the specified user account is invalid. / အသုံးပြုသူ မတွေ့ပါ: ဖော်ပြထားသော user အကောင့် မမှန်ကန်ပါ။'
readonly _ERR_VAR_NAME_='variable not found: the identifier is invalid or does not exist. / အပြောင်းအလဲ မတွေ့ပါ: identifier မမှန်ကန်ပါ သို့မဟုတ် မရှိပါ။'
...
readonly _ERR_ARG_='invalid argument: the argument is not supported by the specified parameter. / အကြောင်းပြချက် မမှန်ကန်ပါ: ဖော်ပြထားသော parameter မှ အဆိုပါ argument ကို မထောက်ခံပါ။'
readonly _ERR_RULE_ALREADY_EXISTS_='falha ao definir: o nome da regra já existe.'
readonly _ERR_HANDLE_EXISTS_='error registering: a handle is already linked to the callback. / မှတ်ပုံတင်ရာ အမှား: callback သို့ handle တစ်ခု ရှိပြီးသား ဖြစ်ပါသည်。'
readonly _ERR_CONNECTION_='connection failure: could not establish connection to Telegram. / ဆက်သွယ်မှု မအောင်မြင်ပါ: Telegram နှင့် ဆက်သွယ်၍ မရပါ။'
...
_eof
	...
	[[ $? -eq 0 ]] && {
		...
		printf '%s was created successfully !! / ဖန်တီးပြီးပါပြီ !!\n' $service
		echo -n "Habilitando..."
